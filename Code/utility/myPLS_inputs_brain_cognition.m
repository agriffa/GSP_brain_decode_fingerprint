...
input.grouping = ones(size(Y,1),1);
...
%input.behav_names = behav_names; 
input.behav_names = ystring; 

% --- Names of the imaging data ---
% will only be used if save_opts.img_type='barPlot'
for i = 1:size(X,2); input.img_names{i,1} = num2str(i); end
%input.img_names = img_names;
...
save_opts.output_path = './PLS_results';
